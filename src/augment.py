...
def simple_paraphrase(text: str) -> str:
    ...
    if not isinstance(text, str) or not text.strip():
        return text
    # Example: synonym-style tweaks and mild rephrasing
    replacements = {
        "study": "trial",
    ...
    for a, b in replacements.items():
        out = out.replace(a, b)
    # small prefix/suffix variations
    variants = [
        out,
...
def augment_dataframe(
    ...
    # Ensure lineage columns exist on originals
    ...
    for _, row in df.iterrows():
        for v in range(1, variants_per_record + 1):
            ...
            # simplistic quality flag—replace with entity checks if available
